...
def stats_str(data):
    if type(data) == dict:  # 如果输入数据是字典类型
        ...
        return f'{mean_orig:.0f} words (original), {mean_samp:.0f} words (sampled), {mean_perturb_orig:.0f} words (perturb original), {mean_perturb_samp:.0f} words (perturb sampled).'  # 返回格式化后的字符串

def save_data(output_file, args, data):
    # 将args写入文件
    args_file = f"{output_file}.args.json"
    with open(args_file, "w") as fout:
        json.dump(args, fout, indent=4)  # 将args以美观格式写入JSON文件
        print(f"Args written into {args_file}")  # 输出提示信息，指示args已写入文件

    # 将data写入保存文件夹中的JSON文件
    data_file = f"{output_file}.raw_data.json"
    with open(data_file, "w") as fout:
        json.dump(data, fout, indent=4)  # 将data以美观格式写入JSON文件
        print(f"Raw data written into {data_file}: {stats_str(data)}")  # 输出提示信息，包括写入的文件名和数据统计信息
